...
export const workspacePackageCommand = async () => {
  ...
  const logger = new Logger(`${cw.color('workspace-package(', primaryLogColor)}${cw.color(packageName, secondaryLogColor)}${cw.color(')', primaryLogColor)}`);
  const workspacePath = process.cwd();
  const { packageMap } = await PackageUtil.getWorkspaceMetadata(workspacePath);
  const localPackage = packageMap[packageName];
  if (!localPackage)
    throw new Error(`Package (${cw.color(packageName, secondaryLogColor)}) does not exist in workspace: ${workspacePath}`)
